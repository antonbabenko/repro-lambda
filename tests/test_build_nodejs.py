...
def test_build_one_nodejs_routes_to_build_nodejs_lambda(git_repo_with_nodejs_sample: Path, mocker):
...
def test_build_one_lambda_at_edge_uses_us_east_1_bucket(git_repo_with_nodejs_sample: Path, mocker):
    mocker.patch(
        "repro_lambda.build.build_nodejs_lambda",
        side_effect=lambda **kw: kw["out_zip"].write_bytes(b"PK\x05\x06" + b"\x00" * 18),
    )
