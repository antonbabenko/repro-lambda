...
def test_compute_content_hash_with_extras_is_host_path_independent(tmp_path: Path):
    ...
    staged = tmp_path / "src"; staged.mkdir(); (staged / "a.py").write_text("a\n")
    lock = tmp_path / "lock.txt"; lock.write_text("L\n")
    spec = LambdaSpec(
        logical_name="x", source_dir="x", requirements_lock="lock.txt",
        runtime="python3.13", arch="arm64", handler="x.h",
    )
    dir_a = tmp_path / "a"; dir_a.mkdir(); (dir_a / "pj").write_text("{}")
    dir_b = tmp_path / "b"; dir_b.mkdir(); (dir_b / "pj").write_text("{}")

    h1 = compute_content_hash(
        staged, lock, spec, "img", "1",
...
def test_compute_content_hash_extras_collide_only_on_relname(tmp_path: Path):
    ...
    pj_a = tmp_path / "outer" / "package.json"; pj_a.parent.mkdir(); pj_a.write_text("{}")
    pj_b = tmp_path / "inner" / "package.json"; pj_b.parent.mkdir(); pj_b.write_text("{}")
...
def test_compute_content_hash_extras_change_when_content_changes(tmp_path: Path):
    ...
    pj_v1 = tmp_path / "v1" / "package.json"; pj_v1.parent.mkdir(); pj_v1.write_text("{}")
    pj_v2 = tmp_path / "v2" / "package.json"; pj_v2.parent.mkdir(); pj_v2.write_text("{\"a\":1}")

    h1 = compute_content_hash(staged, lock, spec, "img", "1",
                              extra_files=[(pj_v1, "package.json")])
    h2 = compute_content_hash(staged, lock, spec, "img", "1",
                              extra_files=[(pj_v2, "package.json")])
    assert h1 != h2
